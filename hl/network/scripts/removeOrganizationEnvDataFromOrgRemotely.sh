...
for destContainer in $(echo "${destOrgContainers}" | jq -r '. | @base64'); do
        _jq(){
                ...
                # start remote script
                sshpass -p "${!destContainerPasswordVar}" ssh ${!destContainerUsernameVar}@${!destContainerHostVar} "cd ${!destContainerPathVar}hl/network && bash scripts/removeExternalOrgEnvData.sh $DELIVERY_ORG_CONFIG_FILE"
                        result=$?
                        echo $result

                        if [ $result -ne 0 ]; then
                                echo "ERROR: Organization network data is not removed from ${destContainerNameValue} host machine"
                                exit 1
                        fi

                        echo
                        echo "Organization environment data has been successfully removed from ${destContainerNameValue} host machine"
